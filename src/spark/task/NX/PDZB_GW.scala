...
class PDZB_GW(hc: HiveContext, val date: Date) extends Serializable {
  import hc.implicits._

  private val PWYW_DMS_TERMINAL_INFO = hc.read.jdbc(utils.url113,
    "(SELECT * FROM PWYW.PWYW_DMS_TERMINAL_INFO WHERE IF_ZF_GW = '1')", utils.cp).persist

  private val PWYW_DMS_BRK_INFO = hc.read.jdbc(utils.url113,
    "(SELECT * FROM PWYW.PWYW_DMS_BRK_INFO WHERE IF_ZF_GW = '1')", utils.cp).persist
  ...
  def execute(statType: String) = {
    ...
    val PWYW_DMS_TMNL_COMM_DET = hc.read.jdbc(utils.urlSGPMS, s"""(select COMM_FAC_ID, OCCUR_TIME, STATUS 
      ...
      .groupBy($"COMM_FAC_ID").mapGroups(
        (id, i) => {
          ...
          if (seq.last.STATUS == 4) seq1 += seq.last //last is 4
    ...
    PDZBToOracle.PWYW_DMS_TMNL_COMM_DET(PWYW_DMS_TMNL_COMM_DET, s"PWYW_DMS_TMNL_COMM_DET_GW$tableName", TJRQ, statType)
    ...
    PDZBToOracle.PWYW_DMS_YK_DET(PWYW_DMS_YK_DET, s"PWYW_DMS_YK_DET_GW$tableName", TJRQ, statType)
    //    PWYW_DMS_YK_DET.show
    //        PWYW_DMS_YK_DET.printSchema()

    val zt1UDF = udf((content: String) => content.split("\\s+")(3))
    ...
    val PWYW_DMS_YX_DET = hc.read.jdbc(utils.urlSGPMS, s"""(select YX_ID as YX_ID1, OCCUR_TIME as YXSJ, CONTENT from yx_bw where occur_time >= 
      ...
      .join(hc.read.jdbc(utils.urlSGPMS, s"""(select YX_ID AS YX_ID2, OCCUR_TIME as SOE_SJ, CONTENT as SOE_CONTENT from yx_soe where occur_time >= 
      to_date('$date1','yyyymmdd') and occur_time < to_date('$date2','yyyymmdd'))""", utils.cpSGPMS)
        .withColumn("SOE_DZ", zt1UDF($"SOE_CONTENT")).filter("SOE_DZ in ('分闸(SOE)', '合闸(SOE)')").withColumn("SOE_SJ1", tsToLongUDF($"SOE_SJ")),
        $"YX_ID1" === $"YX_ID2" and timeUDF($"YXSJ", $"SOE_SJ") and likeUDF($"SOE_DZ", $"YX_DZ"), "left_outer")
      .withColumn("fuck", row_number.over(Window.partitionBy($"YX_ID1").orderBy(expr("ABS(SOE_SJ1 - YXSJ1)"))))
    ...
    PDZBToOracle.PWYW_DMS_YX_DET(PWYW_DMS_YX_DET, s"PWYW_DMS_YX_DET_GW$tableName", TJRQ, statType)
    ...
    PDZBToOracle.PWYW_DMS_KXZDH_DET(PWYW_DMS_KXZDH_DET, s"PWYW_DMS_KXZDH_DET_GW$tableName", TJRQ, statType)
    ...
    PDZBToOracle.PWYW_DMS_XLZDH_DET(PWYW_DMS_XLZDH_DET, s"PWYW_DMS_XLZDH_DET_GW$tableName", TJRQ, statType)
    ...
    PDZBToOracle.PWYW_DMS_XLGZ_DET(PWYW_DMS_XLGZ_DET, s"PWYW_DMS_XLGZ_DET_GW$tableName", TJRQ, statType)
    ...
    PDZBToOracle.PWYW_DMS_YXZB(prov1._1.unionAll(city1._1).unionAll(town1._1).unionAll(bz._1), s"PWYW_DMS_YXZB_GW$tableName", TJRQ, statType)
    PDZBToOracle.PWYW_DMS_JSGLZB(prov1._2.unionAll(city1._2).unionAll(town1._2).unionAll(bz._2), s"PWYW_DMS_JSGLZB_GW$tableName", TJRQ, statType)
