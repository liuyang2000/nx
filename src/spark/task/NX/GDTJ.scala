...
case class QXGDLOG(OBJ_ID: String, JDDJSJ: java.lang.Long, JSSJ: java.sql.Timestamp)

case class QXGDDETAIL(OBJ_ID: String, TZQXSJ: java.lang.Long, JLDDSJ: java.lang.Long, BXSJ: java.lang.Long, JDDJSJ: java.lang.Long, GDSJ: java.lang.Long,
                      JLXFSJ: java.lang.Long, QXGC: String, JSSJ: java.sql.Timestamp)

class GDTJ(hc: HiveContext, val date: Date) extends Serializable {
  ...
  def execute(statType: String) = {
    ...
    val BZQXGD = T_YJ_GZQX_QXD.join(T_YJ_GZQX_QXPGXX, $"OBJ_ID" === "QXD_ID").select($"OBJ_ID", $"BZSSGDDW" as "DWBM", expr("CASE WHEN LY = '04' THEN '1' ELSE '2' END") as "GDLY",
      expr("CASE WHEN GZWHCD = '01' THEN '1' WHEN GZWHCD = '02' THEN '2' WHEN GZWHCD = '03' THEN '3' ELSE GZWHCD END") as "YXFW", $"QXDZT",
      tsToLongUDF($"JLXFSJ") as "JLXFSJ", tsToLongUDF($"JDDJSJ") as "JDDJSJ", tsToLongUDF($"JLDDSJ") as "JLDDSJ", tsToLongUDF($"BXSJ") as "BXSJ",
      tsToLongUDF($"GDSJ") as "GDSJ", tsToLongUDF($"TZQXSJ") as "TZQXSJ")
      .persist  
    ...
    def statBZ = {
      ...
      stat(fun(BZQXGD), fun(JXGD), fun(XJGD)).withColumn("DWJB", lit("8"))
    }
    ...
    def statDetail = {
      val QXGDDETAIL = QXGD.select("OBJ_ID", "TZQXSJ", "JLDDSJ", "BXSJ", "JDDJSJ", "GDSJ", "JLXFSJ")
        .join(T_YJ_GZQX_QXGC.select($"QXD_ID" as "OBJ_ID", $"QXGC"), Seq("OBJ_ID"), "right_outer")
        .join(T_YJ_GGSJ_SERVICE_LOG.select($"YWZJ" as "OBJ_ID", $"JSSJ"), Seq("OBJ_ID"), "right_outer").as[QXGDDETAIL]
        .groupBy($"OBJ_ID")
        .mapGroups(
          (id, i) => {
            val seq = i.toSeq.sortWith((a, b) => a.JSSJ.compareTo(b.JSSJ) > 0)
            ...
            (id.getString(0), SFZP, JDCGSJ, DDXCSC, JDSC, GDCLSC, QXSC)
          })
        .toDF.toDF("OBJ_ID", "SFZP", "JDCGSJ", "DDXCSC", "JDSC", "GDCLSC", "QXSC")

      T_YJ_GZQX_QXD.join(QXGDDETAIL, Seq("OBJ_ID"), "left_outer")
    }
    ...
    NXToOracle.gdDetail(statDetail, date1, statType)
